...
Phase C
 - [x] Onboarding 新手引導：新使用者登入後的 7 步驟導覽流程（包含 KB 命名、檔案匯入、AI Key 設定）。
 - [x] Onboarding Step 6 整合 Review Queue：允許使用者在引導過程中直接審核擷取結果。
 - [x] Ingest Flow 優化：在 Onboarding 內加入 AI 解析中的動態進度條。
            ← SQL function 已就緒
  └── MCP 寫入工具
