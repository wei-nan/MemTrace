...
| **`ws_spec0001`** (live KB) | A running, public workspace seeded from this template, then grown over time with AI-extracted nodes | Database, applied manually via `packages/api/seeds/spec_kb.sql` (see `docs/DEPLOYMENT.md`) | Any logged-in user — appears in their workspace list |
...
3. **Seeding Template** — `packages/api/seeds/spec_kb.sql` was generated from this template (with later AI-extracted additions). To bootstrap a *fresh* spec-as-KB without the historical drift, re-import directly from these JSON files.
