...
- **目標**：引導使用者提供自有金鑰，並在點數耗盡時限制使用。
- [x] **金鑰狀態偵測**：串接 `CreditStatus` 判斷當前 Provider 是否已有使用者金鑰。
- [x] **動態提示橫幅**：若未提供金鑰，在輸入框上方顯示「使用系統點數」的註記與「去設定」連結。
- [x] **點數耗盡鎖定**：若點數為 0 且無自有金鑰，停用發送功能並顯示警告。
