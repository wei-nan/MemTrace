...
>
> **分母已凍結（2026-07-28）**：`docs/SPEC.md` 降級為凍結快照，不再隨產品行為
>更新（見該檔頂部橫幅、`ws_spec_plan/mem_661845d4`）。本文件下方 §2026-05-08
> 根因分析已自承「SPEC.md 增大 2x → 效率分子增大 2x」（見「SPEC.md 增大 2x」
> 一列）——分母膨脹會讓效率倍數虛增，凍結後此問題不再發生，數字更可比較。
> 舊有效率倍數（10.6x / 4.5x 等）仍是歷史量測結果，不因凍結而重新計算。
