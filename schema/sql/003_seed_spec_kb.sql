...
INSERT INTO memory_nodes
  (id,schema_version,workspace_id,title_zh,title_en,content_type,content_format,
   ...
   votes_up,votes_down,verifications,traversal_count,unique_traverser_count)
VALUES
  ('mem_i001','1.0','ws_spec0001','使用者認證：Email 或 Google OAuth','User authentication: email or Google OAuth','procedural','plain',
   '兩種登入路徑，產生相同的 User 物件（id: usr_<hex8>）。Email + Password：bcrypt 雜湊（cost ≥ 12）、密碼政策（8–128 字元、大小寫+數字、HaveIBeenPwned 洩漏檢查）、email 驗證信（24 小時效期）、失敗 5 次鎖定 15 分鐘。Google OAuth 2.0：server-side Authorization Code Flow，ID token 在後端驗證（Google JWKS），state 參數防 CSRF。帳號比對：Google sub 已存在 → 登入；email 與密碼帳號吻合 → 合併；全新 → 建立帳號。Session：JWT（7 天），透過 Authorization: Bearer 傳遞，登出以 blocklist 立即失效。','Two login paths, both producing the same User object (id: usr_<hex8>). Email + Password: bcrypt hash (cost ≥ 12), password policy (8–128 chars, upper+lower+digit, HaveIBeenPwned check), verification email (24h expiry), 5-failure lockout for 15 minutes. Google OAuth 2.0: server-side Authorization Code Flow, ID token validated on backend (Google JWKS), state parameter prevents CSRF. Account matching: Google sub exists → login; email matches password account → merge; new → create account. Session: JWT (7 days), passed via Authorization: Bearer, logout immediately invalidated via blocklist.',ARRAY['auth', 'security', 'oauth', 'jwt']::text[],'public',
...
# 終端 1：FastAPI（含 decay 排程）
cd packages/api && uvicorn main:app --reload --port 8000

# 終端 2：Vite 前端（proxy 已設定至 localhost:8000）
...
VALUES
  ('mem_w004','1.0','ws_spec0001','開發實作順序','Development Implementation Order','procedural','markdown',
   'MemTrace 的建議開發順序（從底層往上）：

1. **core** — 型別、decay、ID/簽章
2. **SQL schema** — 001_init.sql，建立全部資料表
3. **api/core** — config、database、security、deps
4. **api/routers/auth** — register、login、Google OAuth、JWT
5. **api/routers/kb** — workspace、node、edge CRUD + traversal
6. **cli** — init、new、link、list、export、import、copy-node
7. **Seed** — 載入 spec-as-kb 驗證 API 可讀寫（`scripts/seed_spec_kb.py`）
8. **ui/api.ts** — TypeScript API client
9. **ui/AuthPage** — 登入 / 註冊 / Google OAuth
10. **ui/NodeEditor** — 節點編輯器（含 edge panel、節點搜尋）
11. **ui/GraphView** — 2D 知識圖，雙擊開啟 NodeEditor
12. **ui/App** — 串接 auth、workspace selector、side panel
13. **MCP server** — `packages/mcp/`，目前實作 4 個查詢工具（search_nodes、get_node、traverse、list_by_tag）；透過 `.mcp.json` 註冊至 Claude Code

**當前已完成階段：1–13（MCP 僅查詢端）**

**下一步優先項目**：
- 權限模型實作（viewer / contributor / admin 角色強制、contributor 提案流程）
- 工作區軟刪除（status 欄位、30 天排程清除）
- 對話式問答 API（`/chat` endpoint + chat_sessions / chat_messages 資料表）
- AI 攝入流程（`routers/ai.py` 完整化）
- Copy Node API
- Semantic search endpoint
- Workspace 邀請連結 API','Recommended build order for MemTrace (bottom-up):
...
8. **ui/api.ts** — TypeScript API client
9. **ui/AuthPage** — login / register / Google OAuth
