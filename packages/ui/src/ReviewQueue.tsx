    ...
    <div className="review-queue-container" style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <h2 style={{ fontSize: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 13, background: 'var(--bg-surface)', padding: '2px 8px', borderRadius: 10, color: 'var(--text-muted)' }}>
            {items.length}
          </span>
        </h2>
        <div style={{ display: 'flex', gap: 10 }}>
          {items.length > 0 && (
            <>
              <button onClick={handleRejectAll} className="btn-secondary" style={{ color: 'var(--color-error)', borderColor: 'var(--color-error)', padding: '6px 14px' }}>
                {zh ? '全部拒絕' : 'Reject All'}
              </button>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 80, border: '2px dashed var(--border-default)', borderRadius: 16 }}>
            ...
            <div key={item.id} className="review-card" style={{
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {item.node_data.tags?.map((t: string) => (
                  <span key={t} style={{ fontSize: 10, background: 'var(--color-primary-subtle)', color: 'var(--color-primary)', padding: '2px 6px', borderRadius: 4 }}>
                    #{t}
                  </span>
                ))}
              </div>
              
              <div style={{ marginTop: 'auto', paddingTop: 12, borderTop: '1px solid var(--border-default)', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <button onClick={() => handleReject(item.id)} className="btn-icon" title={zh ? '拒絕' : 'Reject'} style={{ color: 'var(--color-error)' }}>
                  <X size={18} />
                </button>
